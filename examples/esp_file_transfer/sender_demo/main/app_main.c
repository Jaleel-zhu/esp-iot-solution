                                   ...
                                   struct ble_gatt_attr *attr, void *arg)
{
    ...
    esp_err_t err = file_transfer_example_link_up(context->session);
    if (err != ESP_OK) {
        ...
        ESP_LOGI(TAG, "Sender link ready, conn=%u, mtu=%u",
                 conn_handle, context->session->mtu_size);
    }
    free(context);
    return 0;
}
                       ...
                       ble_connection_t *connection, int status, void *arg)
{
    ...
    gatt_discovery_discover_service_by_uuid(discovery,
                                            &s_service_uuid.u);
}
