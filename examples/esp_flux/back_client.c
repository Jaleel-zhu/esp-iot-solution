...
static void app_ble_conn_event_handler(void *handler_args, esp_event_base_t base, int32_t id, void *event_data)
{
    ...
    case ESP_BLE_CONN_EVENT_CONNECTED:
        ...
        {
            uint16_t conn_handle = ((esp_ble_conn_event_data_t *)event_data)->connected.conn_handle;
            /* Connection successfully established. */
            /* XXX Set packet length in controller for better throughput */
            int rc = ble_hs_hci_util_set_data_len(conn_handle, LL_PACKET_LENGTH, LL_PACKET_TIME);
            if (rc != 0) {
                ESP_LOGE(TAG, "Set packet length failed; rc = %d", rc);
            }

            rc = esp_ble_conn_mtu_update(conn_handle, OPTIMAL_MTU_SIZE);
            if (rc != ESP_OK) {
                ESP_LOGE(TAG, "Set MTU failed; rc = %d", rc);
            }

            rc = esp_ble_conn_set_preferred_phy(conn_handle, BLE_HCI_LE_PHY_2M_PREF_MASK, BLE_HCI_LE_PHY_2M_PREF_MASK, 0);
            if (rc != ESP_OK) {
                ESP_LOGE(TAG, "Set preferred phy failed; rc = %d", rc);
            }
